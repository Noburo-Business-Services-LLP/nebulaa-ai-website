...
const AGENT_STYLE: Record<string, string> = {
  Gravity: 'bg-brand-gold/15 text-brand-gold',
  ...
  return (
    <main className="bg-[#0A0A0A] text-[#F5F4F1] min-h-screen">
      ...
      <section className="px-6 md:px-12 lg:px-[120px] pt-[140px] pb-[90px] max-w-[900px]">
        ...
        <p className="text-[17px] leading-[1.65] text-white/55 max-w-[620px] mb-9">
          {data.subheadline}
        </p>
        <a
          href="/#pricing"
          className="inline-block bg-brand-gold text-[#1A1208] text-[15px] font-semibold px-[28px] py-[14px] rounded-full shadow-[0_6px_26px_rgba(245,166,35,0.24)] hover:brightness-105 transition"
        >
          Start free trial →
        </a>
      </section>
      ...
      {/* Working with */}
      <div className="border-y border-white/[0.06] py-10 px-6 md:px-12 lg:px-[120px] flex items-center gap-11 flex-wrap">
        ...
        <div className="flex items-center gap-9 flex-wrap">
          {data.clients.map((client) => (
            <span
              key={client.name}
              className={`font-heading text-lg ${client.stage === 'proposal' ? 'text-white/30' : 'text-white/[0.62]'}`}
            >
              {client.name}
      ...
      <section className="px-6 md:px-12 lg:px-[120px] py-[100px]">
        ...
        <div className="grid sm:grid-cols-2 gap-3.5">
          {data.painPoints.map((pain, i) => (
            <div key={i} className="flex items-start gap-3 bg-[#151515] border border-white/[0.06] rounded-xl p-[18px]">
              <X size={15} className="text-red-400/70 mt-0.5 flex-shrink-0" />
              <p className="text-[14.5px] leading-[1.6] text-white/70">{pain}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 md:px-12 lg:px-[120px] py-[100px]">
        <h2 className="font-heading font-medium text-[28px] md:text-4xl mb-8">
          How Nebulaa helps <span className="italic text-brand-gold">{data.name.toLowerCase()}</span>
        </h2>
        <div className="flex flex-col gap-4">
          {data.useCases.map((uc, i) => (
            <div key={i} className="bg-[#151515] border border-white/[0.06] rounded-2xl p-6 flex items-start gap-5">
              ...
              <div>
                ...
                <p className="text-[14.5px] leading-[1.6] text-white/55">{uc.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Close */}
      <section className="relative px-6 md:px-12 lg:px-[120px] py-[130px] pb-[140px] text-center border-t border-white/[0.06] overflow-hidden">
        <div
        ...
        <div className="relative">
          <h2 className="font-heading font-medium text-[32px] md:text-[50px] leading-[1.1] tracking-[-0.02em] mb-6">
            ...
            <br />
            <span className="italic text-brand-gold">Ready in a minute.</span>
          </h2>
          <p className="text-[16px] leading-[1.65] text-white/55 max-w-[480px] mx-auto mb-9">
